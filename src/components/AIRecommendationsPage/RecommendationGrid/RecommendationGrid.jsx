...
export default function RecommendationGrid({ recommendations, loading }) {
  if (loading) {
    return (
  ...
  return (
    <motion.section className={styles.grid} initial="hidden" animate="visible" variants={fadeStagger}>
      {recommendations.map(({ id, title, description, image, tags }) => (
        <motion.article key={id} className={styles.card} variants={fadeUpVariant} whileHover={{ y: -6, boxShadow: "0 10px 30px rgba(0,0,0,0.12)" }}
          ...
          <div className={styles.tags}>
            ...
            <button type="button" className={styles.saveBtn} aria-label={`Save recommendation: ${title}`}>
              <Save size={16} />
            </button>
          </div>
          <button className={styles.viewBtn} type="button">View Details</button>
        </motion.article>
      ))}
    </motion.section>
  );
